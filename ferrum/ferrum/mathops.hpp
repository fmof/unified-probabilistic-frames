#ifndef FERRUM_MATH_OPS_H_
#define FERRUM_MATH_OPS_H_
...
namespace mathops {
  ...
  inline void random_choose(void * dest, size_t k, void * src, 
    ...
    random_choose(dest, k, src, n, size, rnd_gen);
  }
}
...
namespace ferrum {
  ...
  public:
    ...
    ThreadRng& operator=(const ThreadRng&) = delete;
    ThreadRng(ThreadRng&&) = delete;
    ThreadRng& operator=(ThreadRng&&) = delete;
    gsl_rng* get();
  private:
    gsl_rng* r_;
  };
  ...
  namespace functor {
    ...
    class Gaussian1D {
    public:
      Gaussian1D(const gsl_rng*);
      template <typename Index>
      inline const double operator()(Index, Index = 0) const;
    private:
      const gsl_rng* rg_;
      double sigma_;
    };
