...
namespace dmc {
  ...
  void
  dirichlet::sample(const gsl_rng* rng, size_t K,
		    const std::vector<double>& hypers,
		    Eigen::VectorXd& sampled) {
    gsl_ran_dirichlet
      (
       rng, K,
       const_cast<double*>(hypers.data()),
       sampled.data()
       );
  }

  void dirichlet::fisher
  (const std::vector<double>& hypers,
   Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& fish) {
    const double hsum = ferrum::sum(hypers);
    const double trigsum = gsl_sf_psi_1(hsum);
    const size_t K = hypers.size();
    fish.resize(K, K);
    fish.setConstant(-trigsum);
    for(size_t k = 0; k < K; ++k) {
      fish(k, k) += gsl_sf_psi_1(hypers[k]);
    }
  }
  void dirichlet::fisher
  (const double* hypers,
  ...
  void dirichlet::variance(const std::vector<double>& hypers, Eigen::VectorXd& var) {
    const double hsum = ferrum::sum(hypers);
    const double hsumsq = hsum*hsum;
    const size_t K = hypers.size();
    var.resize(K);
    for(size_t k = 0; k < K; ++k) {
      double num = hypers[k] * (hsum - hypers[k]);
      double denom = hsum*hsum*hsum + hsumsq;
      var(k) = num / denom;
    }
  }

  // double dirichlet::log_partition(const std::vector<double>& params, const unsigned int M) {
  //   const double psum = ferrum::sum(params);
  ...
  double cat::entropy(const std::vector<double>& p) {
    double h = 0.0;
    for(double pi : p) {
      if(std::abs(pi) < 1E-10) continue;
      h += pi * gsl_sf_log(pi);
    }
    return -h;
  }

  double dirichlet::entropy(const std::vector<double>& params) {
    const double hsum = ferrum::sum(params);
    const double digsum = gsl_sf_psi(hsum);
    const double lgsum = gsl_sf_lngamma(hsum);
    double h = -lgsum;
    for(const auto& p : params) {
      // grad-log-partition part
      double x = gsl_sf_psi(p) - digsum;
      x *= -(p - 1.0);
      h += x;
      // log-partition part
      double y = gsl_sf_lngamma(p);
      h += y;
    }
    return h;
  }

  void dirichlet::mean(const std::vector<double>& params, std::vector<double>* m) {
    const double hsum = ferrum::sum(params);
    const size_t K = params.size();
    if(m == NULL || m->size() != K) {
      ERROR << "There's an error with the size";
      throw 10;
    }
    cblas_dcopy(K, params.data(), 1, m->data(), 1);
    cblas_dscal(K, hsum, m->data(), 1);
  }

  void dirichlet::grad_mean(const std::vector<double>& params, Eigen::MatrixXd& g) {
    const double hsum = ferrum::sum(params);
    const size_t K = params.size();
    if(g.rows() != (int)K || g.cols() != (int)K) {
      ERROR << "There's an error with the size";
      throw 10;
    }
    const double hsum_sq = hsum * hsum;
    const double hsum_inv = 1.0/hsum;
    for(size_t j = 0; j < K; ++j) {
      double subt = params[j]/hsum_sq;
      g.col(j).setConstant(-subt);
      g(j,j) += hsum_inv;
    }
  }

  double dirichlet::hyperparameters_variational_objective(const std::vector<double>& trial, ClosureType* closure) {
    const double psum = ferrum::sum(trial);
    double individual_sum = 0.0;
  ...
  void dirichlet::hyperparameters_variational_gradient(const std::vector<double>& trial, ClosureType* closure, 
    ...
    ferrum::scalar_product(M, &grad);
    for(const auto& vec : *(closure->variational_params)) {
      std::vector<double> var_grad = dirichlet::grad_log_partition_static(vec);
      ferrum::sum_in_first(&grad, var_grad);
    }
  }
  double dirichlet::hyperparameters_variational_lazy_hessian(const std::vector<double>& trial_weights,
    ...
    const double sum = ferrum::sum(trial_weights);
  ...
  //     ferrum::sum(-1*hessian_odiag, &hessian_diag);
  //     // compute "c"
  ...
  //     ferrum::scalar_product(-1.0 * c, &grad);
  //     for(size_t i = 0; i < dim; ++i) {
  // 	grad[i] /= hessian_diag[i];
  // 	INFO << "multiplier for point " << i << ", val of " << point[i]  << " is " << grad[i] ;
  //     }
  //     ferrum::linear_combination_in_first(&point, grad, 1.0, -1.0);
  //     ferrum::ensure_min(1E-8, &point);
  ...
  std::vector<double> dirichlet::hyperparameters_variational_nr(const std::vector<double>& trial_weights,
	...
	break;
      ...
      for(size_t i = 0; i < dim; ++i) {
	alpha_sum += update[i];
	if(std::abs((update[i] - point[i]) / point[i]) >= thres) {
	  keep_going = true;
	}
      }
      if(++iter >= max_iters) {
  ...
  std::vector<double> dirichlet::grad_log_partition_static(const std::vector<double>& hyperparameters) {
    ...
    const double hsum = ferrum::sum(hyperparameters);
    const double digsum = gsl_sf_psi(hsum);
    size_t index = 0;
  ...
  void dirichlet::grad_log_partition_static(const double* hyperparameters, double* grad, size_t K) {
    if(grad == NULL) {
      ERROR << "grad pointer must not be null";
      throw 2;
    }
    double hsum = 0.0;
    for(size_t k = 0; k < K; ++k) {
      hsum += hyperparameters[k];
    }
    const double digsum = gsl_sf_psi(hsum);
    for(size_t k = 0; k < K; ++k) {
      grad[k] = gsl_sf_psi(hyperparameters[k]) - digsum;
    }
  }
  ...
  void dmc::reestimate_hyperparameters_wallach1(const std::vector<std::vector<int> >& counts,
    ...
    const std::vector< std::vector< int > > histogram = ferrum::histogram(counts);
    const std::vector<int> marginals = ferrum::marginals(counts);
    const std::map<int, int> marginal_histogram = ferrum::sparse_histogram(marginals);
    const int K = counts[0].size();
    const std::vector<int> column_max = ferrum::column_max<int>(counts);
    const int max_marginal = ferrum::max(marginals);
  ...
  //   INFO << "Hyperparameter optimization has optimization status " << opt_status;
  //   ferrum::print_1d(hyperparameters_);
  //   recompute_hyperparameter_sum();
  // }
  ...
  void mfgdmc::reestimate_hyperparameters_wallach1(const std::vector<std::vector<std::vector<int> > >& counts,
						   int num_iterations, double floor) {
    const std::vector< std::vector< int > > histogram = ferrum::mf_histogram(counts);
    const std::vector<int> marginals = ferrum::marginals(counts);
    const std::map<int, int> marginal_histogram = ferrum::sparse_histogram(marginals);
