#ifndef FERRUM_LIBNAR_SAGE_TM_VARIATIONAL_TCC_
#define FERRUM_LIBNAR_SAGE_TM_VARIATIONAL_TCC_
...
namespace ferrum {
  ...
  void SageTMVariational::init
  (
   const DiscreteVariationalInitializer& vi,
   const Vocabulary<VocabType>& vocab,
   CorpusT* corpus, // for polymorphism
   const std::shared_ptr<std::vector<double> >& back
   ) {
    ferrum::Timer init_time(__func__);
    this->vi_init_ = vi;
    num_words_ = vocab.num_words();
    background_ = back;
    const auto& reg_how = vi_init_.regularize_how();
    var_topic_word_params_.resize(num_topics_);
    buffer_topic_word_params_.resize(num_topics_);
    omp_set_num_threads(vi_init_.num_threads(true));
#pragma omp parallel for
    for(int t = 0; t < num_topics_; ++t) {
      STopicType v_topic(num_words_, 1.0, reg_how);
      v_topic.sparse(vi_init_.sparse());
      v_topic.sparsity_threshold(vi_init_.threshold());
      v_topic.background(background_);
  ...
  RecordType
  SageTMVariational::populate_obs_lists(const minsky::CountList& doc_multi,
					std::vector<int>& wid,
  ...
  template <typename CorpusT>
  void SageTMVariational::init_batch
  (
   ...
   const StringDiscreteKindPrinter& print_struct
    ...
    // and now initialize
    int num_words = 0;
    typedef typename CorpusT::const_iterator const_iterator;
    for(const_iterator doc_it = corpus->begin();
	doc_it != corpus->end();
	++doc_it) {
      ...
      var_topic_usage_params_.push_back(vi_init_.usage(topic_usage_hypers_, num_words_in_doc));
