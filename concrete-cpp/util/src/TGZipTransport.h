...
#ifndef _THRIFT_TRANSPORT_TGZIPTRANSPORT_H_
#define _THRIFT_TRANSPORT_TGZIPTRANSPORT_H_ 1
...
class TGZipTransportException : public TTransportException {
 public:
  TGZipTransportException(int status, const char* msg) :
    TTransportException(TTransportException::INTERNAL_ERROR,
                        errorMessage(status, msg)),
    zlib_status_(status),
    zlib_msg_(msg == NULL ? "(null)" : msg) {}

  virtual ~TGZipTransportException() throw() {}
 ...
 *
 * There are two kinds of TGZipTransport objects:
 ...
 *
 */
class TGZipTransport : public TVirtualTransport<TGZipTransport> {
 public:
   ...
   */
  TGZipTransport(boost::shared_ptr<TTransport> transport,
  ...
  /**
   * TGZipTransport destructor.
   *
   * Warning: Destroying a TGZipTransport object may discard any written but
 ...
 private:
  ...
  // - Replace TGZipTransport::flush() calls with TGZipTransport::finish()
  ...
  // TGZipTransport(trans, false) still compiles but behaves incorrectly.
 ...
 *
 */
class TGZipTransportFactory : public TTransportFactory {
 public:
  TGZipTransportFactory() {}

  virtual ~TGZipTransportFactory() {}

  virtual boost::shared_ptr<TTransport> getTransport(
                                         boost::shared_ptr<TTransport> trans) {
    return boost::shared_ptr<TTransport>(new TGZipTransport(trans));
  }
};
...
#endif // #ifndef _THRIFT_TRANSPORT_TGZIPTRANSPORT_H_
