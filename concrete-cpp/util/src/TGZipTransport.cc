...
void TGZipTransport::initZlib() {
  ...
  try {
    ...
    rv = inflateInit2(rstream_, 16+MAX_WBITS);
...
inline void TGZipTransport::checkZlibRv(int status, const char* message) {
  if (status != Z_OK) {
    throw TGZipTransportException(status, message);
  }
}

inline void TGZipTransport::checkZlibRvNothrow(int status, const char* message) {
  if (status != Z_OK) {
    string output = "TGZipTransport: zlib failure in destructor: " +
      TGZipTransportException::errorMessage(status, message);
    GlobalOutput(output.c_str());
  }
}

TGZipTransport::~TGZipTransport() {
  int rv;
  rv = inflateEnd(rstream_);
  checkZlibRvNothrow(rv, rstream_->msg);
...
inline int TGZipTransport::readAvail() {
  return urbuf_size_ - rstream_->avail_out - urpos_;
}

uint32_t TGZipTransport::read(uint8_t* buf, uint32_t len) {
  uint32_t need = len;
...
bool TGZipTransport::readFromZlib() {
  assert(!input_ended_);
...
void TGZipTransport::write(const uint8_t* buf, uint32_t len) {
  if (output_finished_) {
...
void TGZipTransport::flushToTransport(int flush)  {
...
void TGZipTransport::flushToZlib(const uint8_t* buf, int len, int flush) {
...
const uint8_t* TGZipTransport::borrow(uint8_t* buf, uint32_t* len) {
  (void) buf;
...
void TGZipTransport::consume(uint32_t len) {
  if (readAvail() >= (int)len) {
    urpos_ += len;
  } else {
    throw TTransportException(TTransportException::BAD_ARGS,
...
void TGZipTransport::verifyChecksum() {
