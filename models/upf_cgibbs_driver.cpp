...
int get_options(int n_args, char** args,
  ...
  desc.add_options()
    ...
    ("gov-kind-vocab", po::value<std::string>()->default_value("gov_kind_vocab"),
     "gov kind vocab name to load from the redis server")
    ("rel-kind-vocab", po::value<std::string>()->default_value("rel_kind_vocab"),
     "rel kind vocab name to load from the redis server")
...
int main(int n_args, char** args) {
  ...
  ferrum::SampleWithKindsEveryIter strategy(1000, 100);
  ferrum::SymmetricHyperparams hyperparams;
  ...
  typedef ferrum::CollapsedGibbsDMCWithKinds Inferencer;
  ...
  if(vm.count("train") || vm["train-from-db"].as<bool>() ) {
	...
	(
	 ...
	 &rel_vocab,
	 tools,
	 false // don't use lexical semantics
	 );
      CHECK_CORPUS_PTR(corpus);
      corpus->multithreaded( main_redis_connection->multithreaded() );
      std::vector<std::string> vnames = {
	vm["gov-kind-vocab"].as<std::string>(), 
	vm["rel-kind-vocab"].as<std::string>(),
	vm["gov-vocab"].as<std::string>(), 
	vm["rel-vocab"].as<std::string>()
      };
      std::vector<SVocab*> vocabs = {&gov_kind_vocab,
				     &rel_kind_vocab,
				     &gov_vocab,
				     &rel_vocab};
      if(unify_train_vocab) {
	std::vector<minsky::MinskyAnnotationWrapper> order;
	for(auto a : {minsky::AnnotationLevel::SEMANTIC, minsky::AnnotationLevel::SYNTAX}) {
	  for(auto p : {minsky::StructureType::PREDICATE, minsky::StructureType::RELATION}) {
	...
	minsky::PredArgEDocVocabUpdater sevu(order);
	corpus->unify_vocabs(vm["train-name"].as<std::string>() + "::unified",
			     vnames, vocabs, sevu,
			     vm["unified-doc-string"].as<std::string>());
      } else {
	for(size_t vi = 0; vi < vnames.size(); ++vi) {
	  corpus->_save_vocab(vnames[vi], *(vocabs[vi]));
	}
      }
    } else {
	...
	arc_bck_val = vm["uniform-arc-background"].as<double>();
      ...
      Inferencer inf(num_templates, num_slots,
		     gov_kind_vocab.num_words(),
		     rel_kind_vocab.num_words(),
		     hyperparams,
		     false // right now, no latent semantic kinds
		     );
      inf.sampling_strategy(&strategy);

      INFO << "Initializing training model";
      inf.init(corpus, gov_kind_vocab, rel_kind_vocab, gov_vocab, rel_vocab);
